...
def _check_conditions_single_day(row):
    ...
    enable_temperature = cond.get("_enable_temperature", True)
    enable_precipitation = cond.get("_enable_precipitation", True)
    enable_wind = cond.get("_enable_wind", True)
    enable_uv = cond.get("_enable_uv", True)
...
def _build_daily_dataframe(
    ...
    for date_str in dates_a_verifier:
        ...
        enable_temperature = conditions_meteo.get("_enable_temperature", True)
        enable_precipitation = conditions_meteo.get("_enable_precipitation", True)
        enable_wind = conditions_meteo.get("_enable_wind", True)
        enable_uv = conditions_meteo.get("_enable_uv", True)
        ...
        conditions_to_check = []
        if enable_temperature:
            conditions_to_check.append(temp_ok)
        if enable_precipitation:
            conditions_to_check.append(precip_ok)
        if enable_wind:
            conditions_to_check.append(vent_ok)
        if enable_uv:
            conditions_to_check.append(uv_ok)

        conformite_globale = all(conditions_to_check) if conditions_to_check else True

        record = {
            "date": date_str,
            "jour_relatif": jour_relatif,
            "temperature_12h": temp,
            "temp_min_requis": temp_min if enable_temperature else None,
            "temp_max_requis": temp_max if enable_temperature else None,
            "temp_ok": temp_ok,
            "precipitations_12h": precip,
            "precip_min_requis": precip_min if enable_precipitation else None,
            "precip_max_requis": precip_max if enable_precipitation else None,
            "precip_ok": precip_ok,
            "vent_12h": wind,
            "vent_min_requis": vent_min if enable_wind else None,
            "vent_max_requis": vent_max if enable_wind else None,
            "vent_ok": vent_ok,
